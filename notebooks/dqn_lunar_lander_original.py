...
class ReplayBuffer:
    ...
    def sample(self, batch_size):
        ...
        dones = torch.FloatTensor(np.array([e.done for e in experiences], dtype=np.float32)).to(
            device
        )
...
def train_dqn():
    ...
    print(
        f"First episode above 200: {next((i + 1 for i, s in enumerate(scores) if s >= 200), 'Never')}"
    )
...
def plot_training_results(scores, avg_scores, losses):
    ...
    score_ma = np.convolve(scores, np.ones(score_window) / score_window, mode="valid")
    ...
    axes[0].plot(episodes, avg_scores, color="crimson", linewidth=2, label="Avg Score (100 eps)")
    axes[0].plot(
    ...
    axes[0].axhline(y=200, color="forestgreen", linestyle="--", label="Solved (200)")
